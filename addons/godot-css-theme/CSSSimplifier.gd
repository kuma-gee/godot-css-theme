...
func simplify(stylesheet: Stylesheet) -> Stylesheet:
	...
	for cls in stylesheet.get_classes():
		...
		var new_props = stylesheet.get_class_properties(cls, Stylesheet.DEFAULT_STATE).duplicate(
			true
		)

		for state in stylesheet.get_class_states(cls):
			var props = stylesheet.get_class_properties(cls, state).duplicate(true)
